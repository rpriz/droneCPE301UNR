...
int counter_channel_1, counter_channel_2, countaer_channel_3, counter_channel_4;
...
void setup(){
  ...
  DDRA = B11000011;                                                                    //Configure digital poort 4, 5, 6 and 7 as output.
  ...
  if(data == 'a'){
    ...
    if(cal_int != 2000){
      ...
      for (cal_int = 0; cal_int < 2000 ; cal_int ++){                                   //Take 2000 readings for calibration.
        ...
        PORTA |= B11000011;                                                             //Set digital poort 4, 5, 6 and 7 high.
        delayMicroseconds(1000);                                                        //Wait 1000us.
        PORTA &= B00111100;                                                             //Set digital poort 4, 5, 6 and 7 low.
...
void esc_pulse_output(){
  zero_timer = micros();
  PORTA |= B11000011;                                            //Set port 4, 5, 6 and 7 high at once
  ...
  while(PORTA >= 16){                                            //Execute the loop until digital port 4 to 7 is low.
    esc_loop_timer = micros();                                   //Check the current time.
    if(timer_channel_1 <= esc_loop_timer)PORTA &= B11111110;     //When the delay time is expired, digital port 4 is set low.
    if(timer_channel_2 <= esc_loop_timer)PORTA &= B11111101;     //When the delay time is expired, digital port 5 is set low.
    if(timer_channel_3 <= esc_loop_timer)PORTA &= B10111111;     //When the delay time is expired, digital port 6 is set low.
    if(timer_channel_4 <= esc_loop_timer)PORTA &= B01111111;     //When the delay time is expired, digital port 7 is set low.
  }
}
